...
import com.tech57.platform.service.FileStorageService;
...
public class DemandeStageController {
    ...
    // --- Admin : telechargement d'un document ---
    @GetMapping("/api/admin/documents/{id}/telecharger")
    public ResponseEntity<Resource> telechargerDocument(@PathVariable Long id) {
        DocumentDemande doc = documentDemandeRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Document introuvable"));
        Resource resource = fileStorageService.charger(doc.getCheminFichier());
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=" + doc.getNomFichier())
                .body(resource);
    }

    // --- Admin : export PDF d'une demande ---
    ...
    // --- Admin : export Excel de la liste des demandes ---
