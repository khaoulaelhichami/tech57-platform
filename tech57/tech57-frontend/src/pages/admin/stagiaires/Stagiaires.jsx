...
export default function Stagiaires() {
  ...
  const loadEncadrants = () => api.get("/admin/encadrants").then((res) => setEncadrants(res.data));

  useEffect(() => { load(); loadEncadrants(); }, []);
                ...
                <div className="modal-content">
                  ...
                  <div className="modal-body">
                    <div className="mb-2">
                      ...
                      <select className="form-select" value={edition.encadrant || ""}
                              onChange={(e) => setEdition({ ...edition, encadrant: e.target.value })}>
                        <option value="">-- Choisir un encadrant --</option>
                        {encadrants.map((enc) => (
                            <option key={enc.id} value={`${enc.prenom} ${enc.nom}`}>
                              {enc.prenom} {enc.nom} ({enc.specialite})
                            </option>
                        ))}
                      </select>
                    </div>
